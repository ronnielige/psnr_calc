...
int alloc_frame(Frame* f, int width, int height, int bit_depth)
{
	...
	f->frame_size  = f->width * f->height * f->pixel_size * 3 / 2;
	f->y_size      = f->width * f->height * f->pixel_size;
	f->uv_size     = f->width * f->height * f->pixel_size / 4;
...
void get_frame_ssd(Frame* ref, Frame* dst, int64_t ssd[])
{
	if (ref->pixel_size == 1) // 8-bit
	{
		...
		int width  = ref->width;
		ssd[CIDX_Y] = get_block_ssd_8bit(ref->yuv[CIDX_Y], dst->yuv[CIDX_Y], width, height);
		ssd[CIDX_U] = get_block_ssd_8bit(ref->yuv[CIDX_U], dst->yuv[CIDX_U], width / 2, height / 2);
		ssd[CIDX_V] = get_block_ssd_8bit(ref->yuv[CIDX_V], dst->yuv[CIDX_V], width / 2, height / 2);
	}
	else if (ref->pixel_size == 2) // 10-bit
	{
		int height = ref->height;
		int width  = ref->width;
		ssd[CIDX_Y] = get_block_ssd_10bit((uint16_t*)ref->yuv[CIDX_Y], (uint16_t*)dst->yuv[CIDX_Y], width, height);
		ssd[CIDX_U] = get_block_ssd_10bit((uint16_t*)ref->yuv[CIDX_U], (uint16_t*)dst->yuv[CIDX_U], width / 2, height / 2);
		ssd[CIDX_V] = get_block_ssd_10bit((uint16_t*)ref->yuv[CIDX_V], (uint16_t*)dst->yuv[CIDX_V], width / 2, height / 2);
	}
}
...
void show_help(void)
{
	printf("calculate psnr for 8-bit or 10-bit yuv(420)\n");
	printf("usage:\n");
	printf("psnr_calc.exe  ref.yuv  dst.yuv  width  height  [bit_depth]  [frame_num]  [ref_skip_frame_num]  [dst_skip_frame_num]\n");
...
void show_parameters(char* str_ref, char* str_dst, int width, int height, int bit_depth, int frame_num, int ref_skip_frame_num, int dst_skip_frame_num)
{
	printf("ref yuv:            %s\n", str_ref);
	printf("dst yuv:            %s\n", str_dst);
	printf("width  :            %d\n", width);
...
int main(int argc, char* argv[])
{
	...
	show_parameters(argv[ARG_REFYUV], argv[ARG_DSTYUV], width, height, bit_depth, frame_num, skip_ref_frames, skip_dst_frames);

	alloc_frame(&ref_frame, width, height, bit_depth);
	alloc_frame(&dst_frame, width, height, bit_depth);
	...
	for (i = 0; i < frame_num; i++)
	{
		...
		frame_psnr[CIDX_Y] = ssd_to_psnr(pixel_max_ssd * ref_frame.width * ref_frame.height,  frame_ssd[CIDX_Y]);
		frame_psnr[CIDX_U] = ssd_to_psnr(pixel_max_ssd * ref_frame.width * ref_frame.height / 4, frame_ssd[CIDX_U]);
		frame_psnr[CIDX_V] = ssd_to_psnr(pixel_max_ssd * ref_frame.width * ref_frame.height / 4, frame_ssd[CIDX_V]);
